...
// Gemini AI Configuration - API Key included directly
const GEMINI_API_KEY = "AIzaSyDYeLxp7Jp5qSypbVBPy9v_XYmz7Sc1qfs";
const GEMINI_BASE_URL = "https://generativelanguage.googleapis.com/v1beta/models/";

// System instruction for all models
const SYSTEM_INSTRUCTION = `You are an AI assistant created by Shaikh Juned (shaikhjuned.co.in). Your primary goal is to provide helpful, accurate, and professional responses to user queries on WhatsApp. You have expertise in various domains, including web development, AI, and general knowledge. Always strive to be informative, engaging, and concise.
...
- Provide current weather information when asked for a specific city (e.g., "weather in London").

When a user asks for weather, respond with the current weather conditions for the specified city. If the city is not provided, ask the user to specify a city.
...
async function transcribeAudioWithGemini(audioBuffer, mimeType) {
    try {
        ...
        const prompt = "Please transcribe this audio message accurately and provide a helpful response.";
...
async function generateSpeech(text, voiceType = 'female') {
    try {
        ...
        // Return null if TTS fails, don't break the flow
        return null;
    }
}
...
async function processAudioMessage(audioBuffer, mimeType, voiceType = 'female') {
    try {
        ...
        // Generate speech from response (optional, don't fail if TTS doesn't work)
        let audioResponse = null;
        try {
            audioResponse = await generateSpeech(textResponse, voiceType);
        } catch (ttsError) {
            console.log("TTS failed, continuing with text response only:", ttsError.message);
        }
