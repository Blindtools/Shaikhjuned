...
const { transcribeAudio, getTranscriptionStatus } = require("./audio-transcription");
...
async function startWhatsApp() {
    ...
    sock.ev.on("messages.upsert", async ({ messages }) => {
        ...
        try {
            ...
            // Handle image attachments (Vision API)
            ...
            // Handle audio attachments (Transcription API)
                ...
                const transcribedText = await transcribeAudio(buffer, mimeType, false); // Set to true to use Google API
                
                // Then send the transcribed text to Gemini AI for response
                if (transcribedText && !transcribedText.includes("development") && !transcribedText.includes("failed")) {
                    const prompt = `User said (transcribed from audio): "${transcribedText}". Please respond appropriately.`;
                    replyText = await getChatResponse(prompt);
                } else {
                    replyText = transcribedText; // Return the transcription message
                }
            }
            // Handle text messages
                ...
                replyText = "Hello! I'm an AI assistant created by Shaikh Juned (shaikhjuned.co.in). I can help you with text messages, analyze images, extract text from PDFs, and transcribe audio messages. How can I assist you today?";
            }
        ...
        <div style="background-color: #1a1a1a; color: white; font-family: Arial, sans-serif; padding: 30px;">
            ...
            <div style="background-color: #2a2a2a; padding: 20px; border-radius: 10px; margin: 20px 0;">
                ...
                <ul style="text-align: left; max-width: 600px; margin: 0 auto;">
                    ...
                    <li>🎵 Audio transcription (coming soon)</li>
                </ul>
            </div>
