...
async function getChatResponse(text) {
    ...
    try {
        ...
        // Extract the 'result' from the API's JSON response
        if (data && data.result) {
            return data.result;
        } else {
...
async function startWhatsApp() {
    ...
    // Updated socket configuration for better stability
    sock = makeWASocket({
        version,
        auth: state,
        logger: pino({ level: 'silent' }), // Use a silent logger
        qrTimeout: 30000, // QR code timeout in milliseconds
    });
    ...
    sock.ev.on("connection.update", async (update) => {
        ...
        if (connection === "open") {
            ...
            // Reconnect on all errors except when logged out
    ...
    sock.ev.on("messages.upsert", async ({ messages }) => {
        ...
        // Handle button responses if needed in the future
        if (msg.message.buttonsResponseMessage) {
            // Logic for button responses can be added here
        }
        ...
        try {
            ...
            await sock.sendPresenceUpdate('composing', remoteJid); // Show "typing..." status

            // Call the function to get the response from the API
            const replyText = await getChatResponse(incomingText);

            // Send the response back to the user with a button
            await sock.sendMessage(remoteJid, { text: replyText });
            
            await sock.sendPresenceUpdate('paused', remoteJid); // Stop "typing..." status
