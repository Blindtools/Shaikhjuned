...
// Text model for regular conversations - using latest free model
...
// Vision model for image analysis - using latest free model
...
async function generateResponse(prompt, imageParts = null) {
    try {
        ...
        if (imageParts && imageParts.length > 0) {
            console.log("Using Gemini 2.0 Flash Vision API for image analysis...");
            result = await visionModel.generateContent([prompt, ...imageParts]);
        } else {
            console.log("Using Gemini 2.0 Flash API for text generation...");
...
async function processAudioMessage(audioBuffer, mimeType, voiceType = 'female') {
    try {
        console.log("Processing audio message with Gemini 2.0 Flash...");
